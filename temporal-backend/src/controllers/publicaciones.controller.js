...
export async function createPublicacion(req, res) {
	...
	createPublicacion_(publicacion).then(
		(data) => {
    ...
    await Promise.all(
      urls.map(async ({ url, interval }) => {
        ...
        try {
          ...
          console.log(`Publicaciones obtenidas de ${url}:`, publications);

          for (const publication of publications) {
            if (!publication.title) {
              console.error("Publicación sin título detectada:", publication);
              continue;
            }

            try {
              const createdPublication = await createPublicacionIfNotExists(
                publication
              );
          ...
          console.error(`Error en el proceso de scraping para ${url}:`, error);
        }
      })
    );
  ...
  try {
    ...
    for (const publication of publications) {
      ...
      if (year_publi && year_publi.split(",")[1]?.trim() >= "2018") {
        ...
        if (!title) {
          console.error("Error: Título no encontrado para una publicación");
          continue;
        }
        ...
        let citations = await page.evaluate(
          (el) => el.querySelector(".gsc_a_at")?.getAttribute("href") || "",
          publication
        );
